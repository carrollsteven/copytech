...
if (!function_exists("str_sub_count")) {
    ...
    function str_sub_count($str = null, $len = 11, $sub = 1)
    {
        ...
        $cnt = strlen($str); $sub = ($sub > 0) ? $sub : 1;
...
if (!function_exists('active_currencies')) {
     ...
     * @since 1.0
     */
    function active_currencies($output = true, $only = '')
    {
        $active_currency = sys_settings('supported_currency', '[]');
        $all_currencies = get_currencies('full', $only);
...
if (!function_exists('get_exchange_rates')) {
    ...
    function get_exchange_rates($type = null, $currency = null, $flush = false)
    {
        ...
        $automic_rate = array_merge(get_automatic_rates(null, $flush), [base_currency() => 1]);
        $manual_rate  = array_merge(get_manual_rates(), [base_currency() => '1']);

        if (empty($type)) {
            $rates[exchange_methods()->AUTOMATIC] = $automic_rate;
            $rates[exchange_methods()->MANUAL] = $manual_rate;
        }
...
if (!function_exists("filtered_countries")) {
    ...
    function filtered_countries($method = null)
    {
        ...
        if ($method && $restriction == 'global') return Cache::get('fitered_countries') ?? filtered_countries();

        $countries = $method ? data_get($method, 'countries', []) : sys_settings('countries', []);

        if (empty($countries)) return config('countries');
