...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('assets/css/'.$style.'.css?ver=120') }}">
@if(sys_settings('ui_theme_skin', 'default')!='default')
    <link rel="stylesheet" href="{{ asset('assets/css/skins/theme-'.sys_settings('ui_theme_skin').'.css?ver=120') }}">
@endif
...
<body class="nk-body npc-cryptlite pg-auth{{ (gui('skin', 'auth')=='dark') ? ' is-dark' : '' }}{{ dark_theme('active') }}"{!! lang_dir() == 'rtl' ? ' dir="rtl"' : '' !!} data-theme="{{ dark_theme('mode') }}">
...
<script src="{{ asset('/assets/js/bundle.js?ver=120') }}"></script>
<script src="{{ asset('/assets/js/app.js?ver=120') }}"></script>
@stack('scripts')
